...
public static class SkinControle
{
    ...
    public static void DuplicateSkinCheck(PlayerControl target)
    {
        ...
        if (hat || skin)
        {
            ...
            if (hat)
                Utils.SendMessageAutoSplit($"重複しているためハットがリセットにされました", true, sendTo: target.PlayerId);
            if (skin)
                Utils.SendMessageAutoSplit($"重複しているためスキンがリセットにされました", true, sendTo: target.PlayerId);
        }
    }
    public static void ProhibitedSkinCheck(PlayerControl target)
    {
        ...
        Utils.SendMessageAutoSplit($"{GetSetTypeName(hat, skin, visor, pet, !Options.NoHat.GetBool())} は設定で禁止されています。\nスキンがリセットにされました", true, sendTo: target.PlayerId);
    }
